...
def app():
    ...
    with st.expander("PFR vs CSTR Comparison"):
        ...
        def cstr_conversion(rt, c_naoh_0, c_ea_0, k):
            if abs(c_naoh_0 - c_ea_0) < 1e-6:
                # Equal initial concentrations case
                def equal_conc_equation(X):
                    return rt - X / (k * c_naoh_0 * (1-X)**2)
                ...
                M = c_ea_0 / c_naoh_0  # ratio of feed concentrations
                
                def diff_conc_equation(X):
                    return rt - (1 / (k * c_naoh_0)) * np.log((M - X) / (M * (1 - X))) / (M - 1)
        ...
        X_cstr = cstr_conversion(residence_time, feed_conc_naoh, feed_conc_ea, k)
        ...
        for rt in residence_times:
            # PFR Conversion
            V = rt * feed_flow_rate
            L = V / tube_cross_area / 1000
            def pfr_ode_rt(z, X):
                return pfr_ode(z,X) * tube_length / L
            z_span_rt = (0, 1)
            sol_rt = solve_ivp(pfr_ode_rt, z_span_rt, [0], dense_output=True, method='RK45')
            pfr_conversions.append(sol_rt.sol(1)[0] * 100)
            X_cstr_rt = cstr_conversion(rt, feed_conc_naoh, feed_conc_ea, k)
